...
def generate_receipt(request, installment_id):
    ...
    student_data = [
        ...
        ["Payment Date", installment.payment_date.strftime('%d %b %Y %I:%M %p')],
        ["Receipt No", f"INV-{installment.id}"]
    ]
    ...
    payment_data = [
        ["Paid Amount", "Total Fees", "Remaining Fees"],
        [
            f"₹ {installment.amount}",
            f"₹ {student.total_fees}",
            f"₹ {student.remaining_fees}"
        ]
    ]

    payment_table = Table(payment_data, colWidths=[180,180,180])
    ...
    if student.remaining_fees == 0:
        status = "<b>Status: FULLY PAID</b>"
    else:
        status = "<b>Status: PARTIAL PAYMENT</b>"
    ...
    elements.append(Paragraph(
        "© 2026 Way To Web – Professional Training Institute",
        styles["Normal"]
    ))
...
def generate_letter(student, letter_type):
    ...
    if letter_type == "joining":
        ...
        You have successfully joined the <b>{student.course}</b> course.
        ...
        Congratulations! You are offered admission in the 
        <b>{student.course}</b> program at Way To Web.

        Please confirm your admission by completing the enrollment process.
        """
